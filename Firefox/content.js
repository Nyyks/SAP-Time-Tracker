...
chrome.storage.local.get(['customUrls', 'customSelectors', 'playMusic', 'musicDataUrl'], (result) => {
  const urlPatterns = result.customUrls && result.customUrls.length ? result.customUrls : [
    'https://*.s4hana.ondemand.com/*'
  ];
  const selectors = result.customSelectors && result.customSelectors.length ? result.customSelectors : [
    '.sapUiLocalBusyIndicatorAnimation.sapUiLocalBusyIndicatorAnimStandard'
  ];
